...
class UsersService {
  ...
  private signRefreshAndAccessToken = async (user_id: string) => {
  ...
  async register(payload: RegisterReqBody) {
    ...
    const [access_token, refresh_token] = await this.signRefreshAndAccessToken(user_id.toString())
  ...
  async verifyEmail(user_id: string) {
    const [token] = await Promise.all([
      this.signRefreshAndAccessToken(user_id),
  ...
  async resendVerifyEmail(user_id: string) {
    ...
    return { message: USER_MESSAGES.EMAIL_RESEND_SUCCESS }
  }
